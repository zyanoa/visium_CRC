...
# Manuscript threshold: retain targets with R^2 >= 0.10.
# mistyR >= 1.1.11 reports intra.R2 and multi.R2 in percent (e.g. 10 = R^2 0.10),
# whereas very old outputs can be on a 0-1 scale. Detect the scale so the biological
# threshold remains R^2 = 0.10 in either case.
R2_THRESHOLD_FRACTION <- 0.10
r2_max <- suppressWarnings(max(R2_data$R2, na.rm = TRUE))
if (!is.finite(r2_max)) stop("No finite multi.R2 values were found.")
R2_threshold <- if (r2_max > 1.5) 100 * R2_THRESHOLD_FRACTION else R2_THRESHOLD_FRACTION
message("Using MISTy multi.R2 threshold: ", R2_threshold,
        if (r2_max > 1.5) " (percentage scale; equivalent to R^2 >= 0.10)" else " (0-1 scale)")
